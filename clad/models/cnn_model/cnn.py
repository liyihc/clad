...
from clad.utils import Bar, progress_bar

from ..common import OneHot, OnehotEmbedding, get_onehot_infos
from ..common.userdata_loader import DataToSequences, UserDataLoader
from .cnn_gm import DeepAnTModel


class DeepAnTCLLModel(nn.Module):
    def __init__(self, input_record_width, onehots: List[OneHot]) -> None:
        ...
        self.conv1 = nn.Conv1d(input_record_width +
                               self.embedding.output_dim, 10, 5, padding=1)
...
class Model(BaseModel):
    ...
    epochs = 64
    train_batch = 64
    test_batch = Field(1024, compare=False)
    user_limit = -1
    fix_anomaly = True
    embedding = True
    ...
    def get_model(self, *args, **kwds):
        return DeepAnTCLLModel(*args, **kwds)

    def test(self, dataset_path: Path, output_path: Path):
        record = pd.read_csv(
            dataset_path / "test-record.csv").to_numpy(np.float32)
    ...
    def _train(self, model: DeepAnTCLLModel, xs: List[np.ndarray], anomaly_generator: Callable[[int], List[np.ndarray]], epoch_bar: Bar):
    ...
    def _test(self, model: DeepAnTCLLModel, xs: List[np.ndarray], xs_: List[np.ndarray]):
        device = torch.device(self.device)
        model.to(device)
...
class ModelPredict(Model):
    ...
    def _test(self, model: DeepAnTModel, xs: List[np.ndarray], xs_: List[np.ndarray]):
        device = torch.device(self.device)
        model.to(device)
